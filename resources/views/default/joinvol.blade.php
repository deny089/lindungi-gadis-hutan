...
<div class="col-md-8 margin-bottom-20"> 
    ...
    <form method="POST" action="{{ url('donate',$response->id) }}" enctype="multipart/form-data" id="formDonation">
    	...
    	<input type="hidden" name="_id" value="{{ $response->id }}">
			...
			<!-- edited cacip -->
				      <div class="input-group-addon addon-dollar">{{$settings->kode_pohon}}</div>
				      <input type="number" min="1" onchange="OperasiPerhitungan();"  autocomplete="off" id="onlyNumber" class="form-control input-lg" name="amount" value="{{ old('donation') }}" placeholder="Minimum {{$settings->min_donation_amount}} trees"> 
				    
				    <div class="input-group-addon addon-dollar" > x {{$settings->currency_symbol}} <input type="text" id="harga" disabled="disabled" value=" {{$response->hargapohon}} " />  
				  </div>
				  <div class="input-group-addon addon-dollar">Total =  {{$settings->currency_symbol}}  <input  type="text" id="total" disabled="disabled" value="0" />
        ...
        <div class="form-group checkbox icheck">
				<label class="margin-zero">
                  ...
                  <div class="box-footer text-center">
                    ...
                    <button type="submit" id="buttonJoin" class="btn-padding-custom btn btn-lg btn-main custom-rounded">Join Volunteer</button>
...
function OperasiPerhitungan(){
	...
	var A2 = document.getElementById("harga").value;
	var A3 = A1 * A2;
