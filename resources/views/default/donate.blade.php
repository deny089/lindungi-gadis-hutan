    	...
    	<input type="hidden" name="_id" value="{{ $response->id }}">
			...
			<div class="form-group">
				    ...
				    @if( $response->cat_id == 1 )
				     <p>Harga Pohon = {{ $settings->currency_symbol.' '.number_format($response->hargapohon) }} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </p> 
			...
			<!-- edited cacip -->
				    ...
				    @endif
				      <input class="form-control" type="text" min="1" onchange="OperasiPerhitungan();"  autocomplete="off" id="onlyNumber"  name="amount" value="{{ old('donation') }}" placeholder="Minimum {{$settings->min_donation_amount}} trees"> 
				    </div>
				  </div>
				  </div>
				  ...
				  <div class="form-group">
					<div class="input-group">
				  	<div class="input-group-addon addon-dollar">Total =  {{$settings->currency_symbol}} </div> <input  class="form-control" type="text" id="total" disabled="disabled" value="0" />
					</div> 
				  </div> 
				  @endif
			<!-- end edited cacip -->
                 ...
                 <!-- Start -->
                    <div class="form-group">
                      <label>{{ trans('auth.full_name') }}</label>
                        <input type="text"  value="@if( Auth::check() ){{Auth::user()->name}}@endif" name="full_name" class="form-control input-lg" placeholder="{{ trans('misc.first_name_and_last_name') }}" required>
                    </div><!-- /. End-->
                    
                    <!-- Start -->
                    <div class="form-group">
                      <label>{{ trans('auth.email') }}</label>
                        <input type="email"  value="@if( Auth::check() ){{Auth::user()->email}}@endif" name="email" class="form-control input-lg" placeholder="{{ trans('auth.email') }}" required>
                    </div><!-- /. End-->

                    <!-- Start -->
                    <div class="form-group">
                      <label>Phone</label>
                        <input type="text"  value="@if( Auth::check() ){{Auth::user()->telpon}}@endif" name="phone" class="form-control input-lg" placeholder="Phone Number">
                    </div><!-- /. End-->
...
<!-- 
		...
		<div class="form-group ">
				<label class="margin-zero">
					<span class="margin-lft5 keep-login-title">Dengan mengklik donasi, Anda setuju dengan <a href="{{ url('page/sadasdasd1212') }}">Syarat dan Ketentuan</a></span>
			</label>
		</div>
                    <!-- Alert -->
    ...
    @if( Auth::check() )
    <a href="{{url('join/'.$id_user.'/'.$id_campaign)}}" class="btn btn-primary btn-lg btn-block custom-rounded" >
    Gabung Aksi
    </a>
    @else
    <a href="{{url('login')}}" class="btn btn-primary btn-lg btn-block custom-rounded" >
    Gabung Aksi
    </a>
    @endif
 ...
 <div class="col-md-4">
	...
	<div class="panel panel-default">
		<div class="panel-body">
			<h3 class="btn-block margin-zero" style="line-height: inherit;">
				<strong class="font-default">{{number_format($jumlahpohon).' '.$settings->kode_pohon}}</strong> 
				<small>dari Goal {{number_format($response->goal).$settings->kode_pohon}}</small>
				</h3>
				...
				<small class="btn-block margin-bottom-10 text-muted">
					{{$percentage }}% {{trans('misc.raised')}} {{trans('misc.by')}} {{number_format($response->donations()->where('confirmed','1')->count())}} {{trans_choice('misc.donation_plural',$response->donations()->count())}}
				</small>						
		</div>
	</div><!-- End Panel -->
	...
	<div class="panel panel-default">
		<div class="panel-body">
			<img class="img-responsive img-rounded" style="display: inline-block;" src="{{url('public/img/transfer.png')}}" />
		</div>
	</div>
